...
| Query languages (`query`/`cquery`/`aquery`: `deps`, `rdeps`, `somepath`, `allpaths`, file→target, `kind`/`attr` filters, `--output`) | **Adopt — shipped, minus the expression language** | `frost query deps/rdeps/somepath/allpaths/owners` reads the stored graph. `owners` is the file→target direction, over declared action inputs including the generated headers a genrule dependency contributes; a header discovered only through a depfile is build state, which `explain` reports. `allpaths` is bounded by `--limit` and says when it stopped, because the count is exponential on stacked diamonds. `--kind` and `--attr NAME=PATTERN` (deps, srcs, outputs, sandbox, timeout) filter every function against closed sets, so a typo fails rather than widening the answer. `--output text\|json\|label-kind\|dot`; `--json` remains the older spelling of `--output json`. The expression language (`deps(//foo) except //bar`) is rejected: composition belongs to the shell. `compdb`, `plan`, `explain` and the Chrome trace cover the common action-query uses, and affected-test selection is an `rdeps` special case. |
...
| Visibility / `package_group` | **Shipped** | A `visibility` list on targets and named `[visibility.*]` groups, enforced at manifest load. Public by default rather than private: frost has workspaces in the world, and `frost lint` names the crossings instead of breaking them. |
...
| `select()` / configurable attributes | **Shipped (tiny subset)** | `[target.NAME.platform.PLAT]` overrides `srcs`/`deps`/`includes` and adds to `cflags`/`ldflags`, keyed on declared platforms only. No predicate language, and a section cannot change a target's kind or outputs — a platform chooses what a target is built from, never what it is. |
...
| bzlmod / external dependency management | **Reject full system; pinned-fetch subset shipped** | Registry+MVS+extensions took Bazel three major versions and still churns (bazel#23023). Root-level `[fetch.NAME]` pins one HTTP(S) tar.gz/ZIP by SHA-256, optionally strips one prefix, publishes atomically into a non-overlapping vendor directory and stores the verified archive in CAS. Targets opt in with `fetches`; every materialized file enters their action keys. `frost fetch` is the only network path (`--force`/`--offline` are explicit), while a build with missing state fails with the exact fetch command to run. There is deliberately no registry, version solver, transitive resolver, Git fetch or implicit build-time download. |
| Persistent workers, dynamic execution | **Defer — measured, design fixed** ([31](31_persistent_workers.md)) | Measured for javac and tsc: a warm worker removes most of a cold JVM/Node compiler's cost, while native TypeScript 7 removes it without one. Workers wait for per-request input materialization, since the bwrap sandbox is per-process; when they land they speak Bazel's worker protocol, key workers by toolchain closure/environment/cwd, reuse only by input digest and rerun cold under `--check-determinism`. Dynamic execution stays v2 behind remote `Execute` (#144), per-action cancellation and a determinism-gated publication rule. |
