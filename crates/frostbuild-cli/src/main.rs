...
fn run(cli: Cli) -> Result<i32> {
    ...
    match cli.command {
        ...
        Cmd::CoverageLcov {
            ...
            output,
            gcov,
        } => run_coverage_lcov(&root, &gcda, &objects, &output, &gcov),
        Cmd::PackJar {
            input,
            output,
            main_class,
...
///
/// gcov wants the `.gcda` and its `.gcno` side by side, and they are not: the
/// notes file is written by the compile and lives in the object tree, while
/// the counters are relocated by `GCOV_PREFIX` into a directory frost resets
/// before every run — which is what keeps the numbers from accumulating across
/// executions and the tracefile from differing for a build that did not
/// change. So the notes are staged beside the counters here rather than the
/// counters being left where they land.
///
/// `--json-format --stdout` rather than gcov's default: the default writes
/// `.gcov.json.gz`, and reading that would mean a gzip decoder for data that
/// gcov is willing to hand over uncompressed.
fn run_coverage_lcov(
    root: &Path,
    gcda: &Path,
    objects: &Path,
    output: &Path,
    ...
    let mut coverage = frostbuild_core::coverage::Coverage::default();

    let mut counters: Vec<PathBuf> = walk_files(&gcda_dir)
        .into_iter()
        .filter(|path| path.extension().is_some_and(|ext| ext == "gcda"))
        .collect();
    // Sorted so two runs read the same data in the same order. The emitter
    // sorts its records too, but a stable read order keeps a failure
    // reproducible rather than depending on directory iteration.
    counters.sort();

    let notes: std::collections::BTreeMap<std::ffi::OsString, PathBuf> = walk_files(&objects_dir)
        .into_iter()
    ...
    std::fs::write(&path, coverage.to_lcov())
