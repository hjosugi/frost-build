...
#[cfg(feature = "release-assets")]
pub fn generate_release_assets(output: &std::path::Path, date: &str) -> Result<()> {
    release_assets::generate(output, date)
}
...
fn run(cli: Cli) -> Result<i32> {
    // Updating the executable is machine-level work, not workspace work. It
    // must still run from a removed directory or before frost.toml exists.
    if let Cmd::SelfUpdate { check } = &cli.command {
    ...
    match cli.command {
        ...
        Cmd::SelfUpdate { .. } => unreachable!("self-update returns before workspace loading"),
        Cmd::Pick {
            tests,
            print,
            profile,
