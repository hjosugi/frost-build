...
fn recognized_vite_build(package: &WorkspacePackage) -> bool {
    let Some(script) = package.scripts.get("build") else {
        return false;
    };
    let tokens = script
        .split_whitespace()
    ...
    tokens
        .windows(2)
        .any(|pair| pair[0] == "vite" && pair[1] == "build")
...
fn generate_plan(
    ...
    let mut vite_target_names = BTreeMap::new();
    if vite_builds {
        for package in packages
            .values()
            .filter(|package| recognized_vite_build(package))
        {
            let name = target_name(&package.name, "build")?;
            anyhow::ensure!(
                used_names.insert(name.clone()),
                "npm package/script target names collide after sanitization: {name:?}"
            );
            vite_target_names.insert(package.name.clone(), name);
        }
    }
    anyhow::ensure!(
        !target_names.is_empty() || !vite_target_names.is_empty(),
        if vite_builds {
            format!(
                "none of the requested scripts ({}) or recognized Vite builds exist in the discovered npm workspaces",
                scripts.join(", ")
            )
        } else {
            format!(
    ...
    let mut manifest = format!(
        "# Generated by `frost import-npm` from npm workspace metadata.\n\
         # Validation scripts become test gates. Recognized Vite builds are imported only\n\
         # with --vite-builds and receive an explicit profile-specific output tree.\n\
         # Persistent development processes and node_modules remain npm/Vite-owned.\n\n\
         [toolchain.tools]\nnpm = {}\nnode = {}\n\n",
    ...
    for (package_name, name) in &vite_target_names {
        let package = &packages[package_name];
        let mut inputs = common_inputs.clone();
        inputs.extend(package.own_inputs.clone());
        for dependency in workspace_dependency_closure(package_name, &packages) {
        ...
        let mut dependencies = package
            .ordered_dependencies
            .iter()
            .filter_map(|dependency| vite_target_names.get(dependency).cloned())
            .collect::<Vec<_>>();
        dependencies.sort();
        dependencies.dedup();
        let output_directory = format!("{}/dist/${{config}}", package.relative);
        ...
        manifest.push_str(&format!(
            ...
            toml_array(&[
                "--prefix".to_string(),
                package.relative.clone(),
                "run".to_string(),
                "build".to_string(),
                "--".to_string(),
                "--outDir".to_string(),
                "dist/${config}".to_string(),
            ])?,
            toml_array(&inputs)?,
            toml_array(&[output_directory])?,
        ));
        if !dependencies.is_empty() {
            manifest.push_str(&format!("deps = {}\n", toml_array(&dependencies)?));
        }
    ...
    Ok(ImportPlan {
        ...
        target_count: target_names.len() + vite_target_names.len(),
        gate_count: target_names.len(),
        vite_build_count: vite_target_names.len(),
        scripts,
    })
}
...
pub fn run_import(
    ...
    let plan = generate_plan(root, scripts, vite_builds, npm, node)?;
    if dry_run {
        print!("{}", plan.manifest);
        return Ok(0);
    }
    ...
    println!(
        "frost: wrote {} · {} npm workspaces · {} targets ({} test gates, {} Vite builds; {})",
        path.display(),
        plan.package_count,
        plan.target_count,
        plan.gate_count,
        plan.vite_build_count,
        plan.scripts.join(", ")
    );
    println!("  review generated inputs and outputs, then: frost test --all && frost build");
    Ok(0)
}
...
mod tests {
    ...
    fn imports_workspace_scripts_and_transitive_inputs() {
        ...
        let plan = generate_plan(&root, &[], false, Path::new("npm"), Path::new("node")).unwrap();
    ...
    #[test]
    fn imports_recognized_vite_build_as_an_owned_output_tree() {
        let root = fixture("vite");
        std::fs::write(
            root.join("package.json"),
        ...
        .unwrap();
        std::fs::write(root.join("packages/core/src/index.ts"), "export {};").unwrap();
        std::fs::create_dir_all(root.join("apps/web/src")).unwrap();
        std::fs::write(
            root.join("apps/web/package.json"),
            r#"{"name":"@demo/web","scripts":{"build":"tsc -b && vite build"},"dependencies":{"@demo/core":"*"},"devDependencies":{"vite":"8.1.4"}}"#,
        )
        .unwrap();
        std::fs::write(root.join("apps/web/src/main.ts"), "import '@demo/core';").unwrap();

        let plan = generate_plan(&root, &[], true, Path::new("npm"), Path::new("node")).unwrap();
        assert_eq!(plan.vite_build_count, 1);
        let web = plan
            .manifest
            .split("[target.demo-web-build]")
            .nth(1)
            .unwrap();
        assert!(web.contains("kind = \"command\""), "{web}");
        assert!(
            web.contains(
                "args = [\"--prefix\",\"apps/web\",\"run\",\"build\",\"--\",\"--outDir\",\"dist/${config}\"]"
            ),
            "{web}"
        );
        assert!(
            web.contains("output_dirs = [\"apps/web/dist/${config}\"]"),
            "{web}"
        );
        assert!(web.contains("packages/core/src/**/*"), "{web}");
        std::fs::remove_dir_all(root).ok();
    }

    #[test]
    fn vite_import_rejects_ambiguous_output_or_persistent_builds() {
        let root = fixture("vite-ambiguous");
        std::fs::write(root.join("package.json"), r#"{"workspaces":["apps/*"]}"#).unwrap();
        std::fs::create_dir_all(root.join("apps/web/src")).unwrap();
        std::fs::write(
            root.join("apps/web/package.json"),
            r#"{"name":"web","scripts":{"build":"vite build --outDir custom","watch":"vite build --watch"}}"#,
        )
        .unwrap();
        std::fs::write(root.join("apps/web/src/main.ts"), "export {};").unwrap();

        let error =
            generate_plan(&root, &[], true, Path::new("npm"), Path::new("node")).unwrap_err();
        assert!(
            error.to_string().contains("recognized Vite builds"),
            "{error:#}"
        );
        std::fs::remove_dir_all(root).ok();
    }
    ...
    fn rejects_patterns_that_escape_or_match_nothing() {
        ...
        let error =
            generate_plan(&root, &[], false, Path::new("npm"), Path::new("node")).unwrap_err();
